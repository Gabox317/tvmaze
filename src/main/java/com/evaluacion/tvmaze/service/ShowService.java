...
public class ShowService {
	...
	public List<SearchShowResponseDTO> search(String query) {
		...
		List<Long> showIds = searchResults.stream().map(item -> item.show().id()).toList();

		Map<Long, List<CommentResponseDTO>> commentsByShow = commentRepository.findByShowIdIn(showIds).stream()
				.collect(Collectors.groupingBy(CommentDocument::getShowId,
						java.util.stream.Collectors.mapping(
								comment -> new CommentResponseDTO(comment.getComment(), comment.getRating()),
		...
		return searchResults.stream().map(item -> {
			...
			return new SearchShowResponseDTO(show.id(), show.name(), channel, show.summary(), show.genres(),
					commentsByShow.getOrDefault(show.id(), List.of()));
		}).toList();
	}

	public ShowDetailDTO getShow(Long showId) {

		ShowDetailDTO show = showCacheRepository.findById(showId).map(ShowCacheDocument::getData).orElseGet(() -> {

			ShowDetailDTO apiShow = tvMazeClient.getShow(showId);

			ShowCacheDocument document = new ShowCacheDocument(showId, apiShow);
		...
		List<CommentResponseDTO> comments = commentRepository.findByShowIdOrderByCreatedAtAsc(showId).stream()
				.map(comment -> new CommentResponseDTO(comment.getComment(), comment.getRating())).toList();
